...
dates = Time(np.arange(Time('2025-06-01').jd, Time('2026-01-16').jd, 7), format='jd')
...
with solar_system_ephemeris.set('jpl'):
    ...
    for date in dates:
        uranus = get_body('uranus', date)
        ra_list.append(uranus.ra.hour)      # RA in hours
        dec_list.append(uranus.dec.deg)     # Dec in degrees
...
df.to_csv("astropy_uranus_positions.csv", index=False)

import matplotlib.pyplot as plt

# Load your CSV
df = pd.read_csv("astropy_uranus_positions.csv")
